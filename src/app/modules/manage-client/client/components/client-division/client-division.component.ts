...
@Component({
  selector: "app-client-division",
  templateUrl: "./client-division.component.html",
  styleUrls: ["./client-division.component.scss"],
})
export class ClientDivisionComponent implements OnInit {
  modalRef: BsModalRef;
  ...
  ngOnInit() {
    this.activatedRoute.parent.params.subscribe((data:any)=>{
      ...
      this.getBody.search.client_id = data["id"];
    })
    this.getClientDivision(this.getBody);
    this.buildClientDivisionForm();
  }

  buildClientDivisionForm() {
    this.clientDivisionForm = this.fb.group({
      name: [
        this.selectedDivision ? this.selectedDivision.name : "",
        Validators.required,
      ],
      details: [
        this.selectedDivision ? this.selectedDivision.details : "",
        Validators.required,
      ],
      status: [
        this.selectedDivision ? this.selectedDivision.status : "1",
        Validators.required,
      ],
  ...
  addClientDivision(body) {
    this.clientService.addClientDivisions(body).subscribe(
      (response: CustomResponse) => {
        if (response.status) {
          this.toasterMessageService.showSuccess(
            "Client Division Added successfully"
          );
  ...
  editClientDivision(body): void {
    if (this.clientDivisionForm.pristine) {
      this.modalRef.hide();
      return;
    }
    this.clientService.editClientDivisions(body).subscribe(
      (response) => {
        if (response.status) {
          this.toasterMessageService.showSuccess(
  ...
  deleteClientDivisionById(id) {
    this.clientService.deleteClientDivisions(id).subscribe(
      (response) => {
  ...
  openConfirmationDialogue(dataItem) {
    const clientDivisionId = {
      id: dataItem.client_division_id,
    };
    ...
    this.modalRef.content.onClose.subscribe((confirm) => {
      if (confirm) {
        this.deleteClientDivisionById(JSON.stringify(clientDivisionId));
      }
    });
  }
  ...
  openEditModal(dataItem, template: TemplateRef<any>): void {
    ...
    this.selectedDivision = dataItem;
    this.buildClientDivisionForm();
    this.modalRef = this.modalService.show(template, this.config);
  }

  onSubmit() {
    if (this.clientDivisionForm.invalid) return;
    if (this.editMode) {
      this.clientDivisionForm.patchValue({
        id: this.selectedDivision.client_division_id,
      });
      this.editClientDivision(this.clientDivisionForm.value);
    } else {
      this.addClientDivision(this.clientDivisionForm.value);
    }
  }
}
