...
import { AdditionDeductionService,  } from "../../services/additiondeduction.service";
...
@Component({
  selector: "app-additiondeduction-list",
  templateUrl: "./additiondeduction-list.component.html",
  styleUrls: ["./additiondeduction-list.component.scss"],
})
export class AdditionDeductionListComponent implements OnInit {
  departmentForm: FormGroup;
  modalRef: BsModalRef;
  listLoading = false;
  departmentDetail: any;
  selectedAdditionDeduction: DepartmentListModel;
  submitted: boolean;
  language: any;
  submitButton: string;
  editMode: boolean;
  ...
  constructor(
    ...
    private additionDeductionService: AdditionDeductionService,
  ...
  buildDepartmentForm(): void {
    const department = this.selectedAdditionDeduction;
  ...
  getdepartmentList(): void {
    ...
    this.additionDeductionService.getDepartmentList(params).subscribe(
  ...
  addDepartment(body): void {
    ...
    this.additionDeductionService.addDepartment(bodyParms).subscribe(
      (response: CustomResponse) => {
        if (response.status) {
  ...
  editDepartment(body): void {
    ...
    this.additionDeductionService.editDepartment(body).subscribe(
      (response: CustomResponse) => {
        if (response.status) {
          this.toasterMessageService.showSuccess(
  ...
  deleteDepartmentById(id): void {
    this.additionDeductionService.deleteDepartment(id).subscribe(
      (response: CustomResponse) => {
  ...
  onSubmitDepartment(): void {
    ...
    if (this.editMode) {
      this.departmentForm.patchValue({
        id: this.selectedAdditionDeduction.department_id,
      });
  ...
  openAddModal(template: TemplateRef<any>): void {
    ...
    this.modalTitle = "Add Addition/Deduction";
    this.selectedAdditionDeduction = null;
    this.buildDepartmentForm();
  ...
  openEditModel(department, template: TemplateRef<any>): void {
    ...
    this.modalTitle = "Edit Addition/Deduction";
    this.additionDeductionService.setSelectedDepartment(department);
    this.selectedAdditionDeduction = this.additionDeductionService.getSelectedDepartment();
    this.buildDepartmentForm();
    this.modalRef = this.modalService.show(template, this.config);
  }
}
