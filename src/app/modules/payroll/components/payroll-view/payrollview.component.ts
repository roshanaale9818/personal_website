...
import { PayrollList } from './modal/payrollList.interface';
import { DownloadPayrollCSV } from './modal/downloadpayrollcsv.interface';
import { RoleService } from '@app/core/guards/auth/services/role.service';
...
@Component({
  ...
  styleUrls: ['./payrollview.component.scss']
})
export class PayrollViewComponent implements OnInit {
  ...
  onViewClicked() {
    ...
    // }
    this.params.search.date_from = this._dateFrom;
    this.params.search.date_to = this._dateTo;
    this.params.search.period_id = Number(this._id);
    // console.log("this is limit",this.payrollListBody.limit,this.pageLimit)
    this.payrollListBody.limit = this.pageLimit;
    this.payrollListBody.page = Number(this.params.page);
    this.payrollListBody.access_token = this.globalService.getAccessTokenFromCookie(),
      this.payrollListBody.company_id = this.params.company_id;
    this.payrollListBody.sortnane = "";
    this.payrollListBody.sortno = 1;
    this.payrollListBody.period_id = this.params.search.period_id;

    if (!this.params.search.date_to || this.params.search.date_to.split("-").includes("NaN")) {
      return;
    }
    this.params.search.status = this.status ? "1" : "0";
    this.payrollListBody.search.staff_id = this.searchForm.get('emp_id').value;
    this.payrollListBody.search.status = (this.searchForm.get('status').value) == 'paid' ? 1 : 0;
    if (!(this.searchForm.get('status').value)) {
      this.payrollListBody.search.status = null
    }
    this.listLoading = true;
    this.payrollService.onPayrollList(this.payrollListBody).subscribe((data: CustomResponse) => {
      if (data.status) {
  ...
  configUserDateAndTimeSetting() {
    ...
    this.settingFromCompanyWise = this.localStorageService.getLocalStorageItem(
      ...
      : null;
      console.log(this.dateFormatSetting);
  ...
  onHistoryClicked(value) {
    ...
    this.router.navigate(['/payroll/history', { staffId: value.staff_id, returnUrl: this.router.url,staff_name:
      `${value.emp_id} ${value.first_name}  ${value.middle_name} ${value.last_name}` }]);
  }
  ...
  onOpenAdvancedModal(template: TemplateRef<any>, data, type) {
    ...
    this.totalAmt = Number(data.gross_salary);
    ...
    this.getPaySlipPayOrDeductDetail(type,data)
  ...
  onMultipleAdvanceSave() {
    ...
    const body:any[] = this.advancedForm.get("advancedFormArray").value;
    if (!body) return;
    ...
    let bodyObj = {
      ...
      company_id: this.globalService.getCompanyIdFromStorage(),
      payslip_id: this.selectedAdvanced.payslip_id,
      paydata: sendingArray
    }
    console.log("this is request",bodyObj);
    // return
    this.saving = true;
  ...
  getPaySlipPayOrDeductDetail(type,data?:any) {
    ...
    this.payrollService.getPaySlipAddDeduct(this.selectedAdvanced.payslip_id, Type).subscribe((res: CustomResponse) => {
      if (res.status) {
        ...
        console.log("calling heresdsdsad")
        //call for calculating
        // this.onAmountChange()
      }
    }, err => {
  ...
  onPayrollCSVDownload() {
    let body: DownloadPayrollCSV = {
      access_token: this.globalService.getAccessTokenFromCookie(),
      sortnane: "",
      sortno: 1,
      page: +this.page,
    ...
    this.payrollService.onDownLoadCSV(body).subscribe((res: CustomResponse) => {
      if (res.status) {
        // console.log("res",res)
