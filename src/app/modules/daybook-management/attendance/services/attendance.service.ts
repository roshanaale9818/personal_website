...
export class AttendanceService {
  ...
  requestReviewCheckOut(reqReviewFormData): Observable<any> {
    // console.log("req date",reqReviewFormData.req_date);
    // console.log("datetime",reqReviewFormData.datetime)
    const params = new HttpParams()
      .set("id", reqReviewFormData.id)
      .set("datetime", reqReviewFormData.datetime)
      .set("req_date", reqReviewFormData.req_date)
  ...
  getForgetTocheckoutStatus(id): Observable<any> {
    const params = new HttpParams().set("id", id).set("company_id", this.companyId);
  ...
  getUserDetail(){
    const id = this.globalService.getStaffId()
    const params = new HttpParams().set("id", id);
    return this.httpClientService.get(
      `${this.baseIp}${this.apiPrefix}staff/details`,
      null,
      params
    );
  }
